...
#include "tsan/rtl/ufo/ufo_interface.h"
...
void ThreadContext::OnCreated(void *arg) {
  ...
  aser::ufo::on_thread_created(args->thr->tid, this->tid, args->pc);//JEFF
...
void ThreadContext::OnStarted(void *arg) {
  ...
  aser::ufo::on_thread_start(thr,args->stk_addr, args->stk_size, args->tls_addr, args->tls_size);//JEFF
...
void ThreadFinish(ThreadState *thr) {
  ...
  aser::ufo::on_thread_end(thr);//JEFF
...
void ThreadJoin(ThreadState *thr, uptr pc, int tid) {
  ...
  CHECK_LT(tid, kMaxTid);
   ...
   DPrintf3("#%d: ThreadJoin tid=%d\n", thr->tid, tid);
  
  aser::ufo::on_thread_join(thr->tid, tid, pc);//JEFF

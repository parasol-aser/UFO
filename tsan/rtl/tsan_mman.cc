...
void user_free(ThreadState *thr, uptr pc, void *p, bool signal) {
  ...
  if (ctx && ctx->initialized)
    ...
    //JEFF don't free the memory for experiments
    //allocator()->Deallocate(&thr->proc()->alloc_cache, p);  if (signal)
...
//    Printf("JEFF: 194");
  DPrintf("#%d: alloc(%zu) = %p\n", thr->tid, sz, p);
    aser::ufo::on_alloc(thr,pc,(void *)p,sz);//JEFF
...
void OnUserFree(ThreadState *thr, uptr pc, uptr p, bool write) {
  ...
  uptr sz = ctx->metamap.FreeBlock(thr->proc(), p);
    ...
    aser::ufo::on_dealloc(thr,pc,(void *)p,sz);//JEFF

  DPrintf3("#%d: free(%p, %zu)\n", thr->tid, p, sz);
//  if (write && thr->ignore_reads_and_writes == 0)//JEFF
